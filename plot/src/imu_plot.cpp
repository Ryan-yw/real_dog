...
#include "ui_imu_plot.h"


std::atomic<double> imu_angle[3] = {0,0,0};

ImuPlotWindow::ImuPlotWindow(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::ImuPlotWindow),
    mPlot(nullptr),
    mTag1(nullptr),
    mTag2(nullptr),
    mTag3(nullptr)

{
    ...
    connect(&mDataTimer, SIGNAL(timeout()), this, SLOT(plotImu()));

    mDataTimer.start(40);

}

void ImuPlotWindow::plotImu()
{
